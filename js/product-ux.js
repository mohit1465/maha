...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const socialProofData = [
        { name: 'Rahul', city: 'Delhi', initial: 'R', color: '#e74c3c' },
        { name: 'Priya', city: 'Mumbai', initial: 'P', color: '#9b59b6' },
        { name: 'Amit', city: 'Bengaluru', initial: 'A', color: '#2ecc71' },
        { name: 'Sunita', city: 'Hyderabad', initial: 'S', color: '#3498db' },
        { name: 'Vikram', city: 'Chennai', initial: 'V', color: '#e67e22' },
        { name: 'Meena', city: 'Jaipur', initial: 'M', color: '#1abc9c' },
        { name: 'Rohit', city: 'Pune', initial: 'R', color: '#e91e63' },
        { name: 'Kavya', city: 'Ahmedabad', initial: 'K', color: '#ff5722' },
    ];
    ...
    if (toast && toastClose) {
        ...
        const showSocialToast = () => {
            ...
            toastAvatar.textContent = data.initial;
            toastAvatar.style.background = `linear-gradient(135deg, ${data.color}, ${data.color}cc)`;
            toastName.textContent = `${data.name} from ${data.city}`;
        ...
        // Show first toast after 6 seconds, then every 30 seconds
        setTimeout(showSocialToast, 6000);
        setInterval(showSocialToast, 30000);
    }
    ...
    const initRecentlyViewed = () => {
        ...
        let history = [];
        try {
            history = JSON.parse(localStorage.getItem('maha_recently_viewed') || '[]');
        } catch (e) {}

        // Add current product if not already there
        if (productTitle) {
            history = history.filter(item => item.url !== currentUrl);
            history.unshift({ name: productTitle, url: currentUrl });
            history = history.slice(0, 5); // Keep last 5
            try {
                localStorage.setItem('maha_recently_viewed', JSON.stringify(history));
            } catch (e) {}
        }

        // Show bar if there are previous items (exclude current)
        const previousItems = history.filter(item => item.url !== currentUrl).slice(0, 3);
        const bar = document.getElementById('recentlyViewedBar');
        const itemsContainer = document.getElementById('recentlyViewedItems');
        const closeBtn = document.getElementById('recentlyViewedClose');

        if (bar && itemsContainer && previousItems.length > 0) {
            itemsContainer.innerHTML = '';
            previousItems.forEach(item => {
                const a = document.createElement('span');
                a.className = 'recently-viewed-item';
                a.textContent = item.name.length > 12 ? item.name.substring(0, 12) + '…' : item.name;
                a.title = item.name;
                a.addEventListener('click', () => { window.location.href = item.url; });
    ...
    // Call after product loads (give JS time to populate title)
    setTimeout(initRecentlyViewed, 2000);
